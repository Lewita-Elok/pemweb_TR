  ...
  <?php foreach ($listProduk as $produk): ?>
    <div class="max-w-full md:max-w-[350px] border group rounded-xl transition-all duration-300">
      ...
      <div class="px-5 pt-2 pb-5">
        ...
        <p class="mt-5 text-slate-500">Buket pilihan sesuai dengan gambar. Silahkan tekan icon diatas untuk melihat secara
          detail</p>
...
<section id="ulasan" class="mt-12 px-4 md:px-48">
  ...
  <div
    class="mt-12 <?= (isset($_SESSION['role']) && $_SESSION['role'] == 'customer') ? 'grid grid-cols-1 lg:grid-cols-2' : '' ?>  gap-x-8">
    <div
      class="max-h-96 overflow-y-auto hide-scrollbar <?= (isset($_SESSION['role']) && $_SESSION['role'] == 'customer') ? 'space-y-8' : 'grid grid-cols-1 gap-8 md:grid-cols-3' ?>  ">
